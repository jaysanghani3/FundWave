  ...
  return (
    <div className="text-xs">
      ...
      <div className="flex justify-end items-center space-x-2 mt-3">
        <input type="text" placeholder="Search by name" className="border px-2 py-1" value={searchValue} onChange={(e)=>{setSearchValue(e.target.value);handleSearch()}}/>
        ...
        <Link to="/add-new-cutomer">
          <button className="bg-[#1d5e7e] text-white px-3 py-1">Add New {title.split(' ')[0]}</button>
        </Link>
      </div>
      ...
      <table className="table-auto w-full mt-3 ">
        ...
        <tr className="bg-[#1d5e7e] text-white">
            {tableHeader.map((item, index) => (<td key={index} className={`px-2 py-1 border-r ${item.width}`}>{item.name}</td>))}
        </tr>
        </thead>
        <tbody>
          {
