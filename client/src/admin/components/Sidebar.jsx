        ...
        <div key={index} className="border-b border-[#154056] py-1">
          {menu.menuName == "Dashboard" ? (
            <Link to={menu.link} className="flex items-center w-full focus:outline-none hover:bg-[#267399] text-gray-600 border-l-4 border-transparent hover:border-white pr-6 py-1 flex-row">
              ...
              <span className="text-[16px] hidden sm:block tracking-wide truncate mx-2 text-white font-semibold">{menu.menuName}</span>
            </Link>
          ) : (
            <>
              ...
              {openSubMenu === index &&
                menu.subMenus.map((subMenu, subIndex) => (
                  <Link key={subIndex} to={subMenu.link} className="flex flex-col text-blue-100 text-sm pl-8 pr-2 hover:bg-[#267399] w-full py-1">
                    {subMenu.name}
                  </Link>
                ))}
            </>
